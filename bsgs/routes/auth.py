...
#
# X-BSGS-Pubkey -- Ed25519 pubkey of the user.  If blinded, this starts with "15" and the pubkey is
# the user's blinded bchat id on the BSGS.  If *unblinded* this starts with "00", the remainder is
# an Ed25519 pubkey, and we convert it to an X25519 pubkey to determine the user's bd... bchat id.
#
# X-BSGS-Nonce -- a unique 128-bit (16 byte) request nonce, encoded in either base64 (22 chars (or
...
#
# X-BSGS-Timestamp -- unix integer timestamp, expressed in the usual human (base 10) notation.  The
# timestamp must be with ±24 hours of the BSGS server time when the request is received.
#
# X-BSGS-Signature -- Ed25519 signature (passed in base64 encoding) of:
#
...
#
# For example, for a GET request to '/capabilities?required=bsgs' to a server with pubkey
# fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210 the request headers could be:
#
# X-BSGS-Pubkey: 150123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef
# X-BSGS-Nonce: IYUVSYbLlTgmnigr/H3Tdg==
# X-BSGS-Timestamp: 1642079887
# X-BSGS-Signature: ...
#
# Where ... is the 88-character (including 2 padding chars) base64 encoding of the 64-byte value
# obtained by signing:
#
# b'xxx...xxxYYY...YYY1642079887GET/capabilities?required=bsgs'
#   ^^^^^^^^^#########^^^^^^^^^^###^^^^^^^^^^^^^^^^^^^^^^^^^^^
#     `- server pubkey, 32B   |  |         |                  |
#               `- nonce, 16B |  |         |                  |
...
#
# POST /beldex/v3/lsrpc
#
# with the (post-decryption) body containing:
#
# {"endpoint": "/some/endpoint", "method": "POST", "body": "{\"a\":1}", "headers": {
#   "X-BSGS-Pubkey": "050123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
#   "X-BSGS-Nonce": "5f9369b79449a7dfa07d123b697c84f6",  # random, hex encoded
#   "X-BSGS-Timestamp": "1642080374",
#   "X-BSGS-Signature": "...",
# }}
#
...
#
# For batch requests the X-BSGS-* headers are applied once, on the outermost batch request, *not* on
# the individual subrequests; the authorization applies to all subrequests.
#
# NB: legacy bsgs endpoints (that is: endpoint paths without a leading /) require specifying the
...
def require_user():
    ...
    if g.user is None:
        abort_with_reason(http.UNAUTHORIZED, 'X-BSGS-* request authentication required')
...
def handle_http_auth():
    ...
    # user_reauth has been specifically set (from bsgs.routes.subrequest).
    ...
    pk, nonce, ts_str, sig_in = (
        request.headers.get(f"X-BSGS-{h}") for h in ('Pubkey', 'Nonce', 'Timestamp', 'Signature')
    )
    ...
    if missing:
        abort_with_reason(
            http.BAD_REQUEST,
            f"Invalid authentication headers: missing {missing}/4 required X-BSGS-* headers",
        )
    ...
    try:
        ...
        abort_with_reason(
            http.BAD_REQUEST, "Invalid authentication: X-BSGS-Pubkey is not a valid 66-hex digit id"
        )

    if pk[0] not in (0x00, 0x15):
        abort_with_reason(
            http.BAD_REQUEST, "Invalid authentication: X-BSGS-Pubkey must be 00- or 15- prefixed"
        )
    ...
    if not sodium.crypto_core_ed25519_is_valid_point(pk):
        abort_with_reason(
            http.BAD_REQUEST,
            "Invalid authentication: given X-BSGS-Pubkey is not a valid Ed25519 pubkey",
        )

    pk = VerifyKey(pk)
    if blinded_pk:
        session_id = 'bd' + pk.encode().hex()
        ...
        try:
            session_id = 'bd' + pk.to_curve25519_public_key().encode().hex()
    ...
    try:
        ...
        abort_with_reason(
            http.BAD_REQUEST,
            "Invalid authentication: X-BSGS-Nonce must be 16 bytes (encoded as base64 or hex)",
        )
    ...
    try:
        ...
        abort_with_reason(
            http.BAD_REQUEST, "Invalid authentication: X-BSGS-Signature is not base64[88]"
        )
    ...
    try:
        ...
        abort_with_reason(
            http.BAD_REQUEST, "Invalid authentication: X-BSGS-Timestamp is not a valid timestamp"
        )
    ...
    if not now - 24 * 60 * 60 <= ts <= now + 24 * 60 * 60:
        abort_with_reason(
            http.TOO_EARLY, "Invalid authentication: X-BSGS-Timestamp is too far from current time"
        )
    ...
    try:
        ...
        abort_with_reason(http.TOO_EARLY, "Invalid authentication: X-BSGS-Nonce cannot be reused")
    ...
    try:
        ...
        abort_with_reason(
            http.UNAUTHORIZED, "Invalid authentication: X-BSGS-Signature verification failed"
        )
