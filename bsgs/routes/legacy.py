...
def legacy_check_user_room(
    ...
    request's X-BSGS-* headers (if present) or Authorization header.
    ...
    if not pubkey or len(pubkey) != (utils.SESSION_ID_SIZE * 2) or not pubkey.startswith('bd'):
...
def get_rooms():
    ...
    return jsonify(
        {
            'status_code': http.OK,
            # Legacy Bchat only wants token (returned as 'id') and name:
...
def get_room_info(room):
    """serve room metadata"""
    # This really should be authenticated but legacy Bchat just doesn't pass along auth info.
...
def legacy_transform_message(m):
    """Transform new API fields into legacy Bchat fields"""
    return {
        'server_id': m['id'],
...
def handle_comapct_poll():
    ...
    for req in req_list.get('requests', list()):
        try:
            ...
            # Hack for Bchat: if there isn't a full fleshed out response then Bchat just ignores
...
def process_legacy_file_upload_for_room(user, room, lifetime=config.UPLOAD_DEFAULT_EXPIRY):
    ...
    filename = None  # legacy Bchat doesn't provide a filename, just a random blob
...
# Posting here adds an admin and requires admin access.  Legacy Bchat doesn't understand the
...
def handle_legacy_add_admin():
    ...
    if len(session_id) != 66 or not session_id.startswith("bd"):
        abort(http.BAD_REQUEST)
...
# DELETE here removes an admin or moderator and requires admin access.  (Legacy Bchat doesn't
