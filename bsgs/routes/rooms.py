...
def get_one_room(room):
    ...
    - `created` — Unix timestamp (as a float) of the room creation time.  Note that unlike earlier
      versions of BSGS, this is a proper seconds-since-epoch unix timestamp, not a javascript-style
      millisecond value.
    ...
    - `active_users_cutoff` — The length of time (in seconds) of the `active_users` period.
      Defaults to a week (604800), but the social group administrator can configure it.
    ...
    - `pinned_messages` — Array of pinned message information (omitted entirely if there are no
      pinned messages).  Each array element is an object with keys:
        ...
        * `pinned_by` — The bchat ID of the admin who pinned this message (which is not
          necessarily the same as the author of the message).
    - `moderators` — Array of Bchat IDs of the room's publicly viewable moderators.  This does not
      include room administrators nor hidden moderators.
    - `admins` — Array of Bchat IDs of the room's publicly viewable moderators.  This does not
      include room moderator nor hidden admins.
    - `hidden_moderators` — Array of Bchat IDs of the room's publicly hidden moderators.  This
      field is only included if the requesting user has moderator or admin permissions, and is
      omitted if empty.
    - `hidden_admins` — Array of Bchat IDs of the room's publicly hidden admins.  This field is
...
def get_user_permission_info(room, sid):
    """
    Fetches assigned room permissions/bans/etc. for the given bchat id in the room.
...
def get_user_future_permissions(room, sid):
    ...
    only rows for the given Bchat ID are returned.
...
def set_permissions(room, sid):
    ...
    - `accessible` — can be true or false to grant or revoke access to this room for the given
      bchat id when the bchat id does not have `read` access.  That is: a user with neither
    ...
    - `read` — can be true or false to grant or revoke read permission for the given bchat id in
    ...
    - `write` — can be true or false to grant or revoke write (i.e. posting) permissions for the
      given bchat id in this room.  If omitted the write permission is not changed.

    - `upload` — can be true or false to grant or revoke upload (i.e. attachment) permission for
      this given bchat id in this room.  Note that uploading *also* requires the `write`
    ...
    this bchat ID in the [GET `/room/ROOM/permissions` endpoint](#get-roomroompermissions), but
    with an additional key `session_id` indicating the bchat id to which the permissions were
    applied.

    Note that when blinding is enabled and this endpoint is called with an unblinded bchat ID, the
...
def set_future_permissions(room, sid):
    ...
    scheduled against the *blinded* Bchat ID, if known, rather than the unblinded id.
    """
...
def serve_file_with_ignored_filename(room, fileId, filename):
    ...
    - `filename` — Arbitrary filename of the attachment; this value is entirely ignored by BSGS.
...
def delete_all_posts(room, sid):
    ...
    - `sid` — the bchat id of the user to ban
