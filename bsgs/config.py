...
def load_config():
    if 'BSGS_CONFIG' in os.environ:
        conf_ini = os.environ['BSGS_CONFIG']
        if conf_ini and not os.path.exists(conf_ini):
            raise RuntimeError(f"BSGS_CONFIG={conf_ini} specified, but path does not exist!")
    else:
        conf_ini = 'bsgs.ini'
        if not os.path.exists(conf_ini):
            logger.info("bsgs.ini does not exist; using all config defaults")
            conf_ini = None
