# Migration code for upgrading from a 0.1.x BSGS database.  The database structure is completely
...
def import_from_0_1_x(conn):
    ...
    def ins_user(bchat_id):
        if not db.query(
            "SELECT COUNT(*) FROM users WHERE bchat_id = :bchat_id",
            dbconn=conn,
            bchat_id=bchat_id,
        ).first()[0]:
            db.query(
                "INSERT INTO users (bchat_id, last_active) VALUES (:bchat_id, 0.0)",
                bchat_id=bchat_id,
                dbconn=conn,
            )
    ...
    for room_token, room_name in rooms:
        ...
        with sqlite_connect_readonly(room_db_path) as rconn:
            ...
            #    );
            #
            # where public_key is the bchat_id (in hex), timestamp is in milliseconds since
            ...
            #    );
            #
            # where the `id` of this table is returned to the Bchat client so that they can
            # query for "deletions since [id]".
            #
            # This introduces some major complications, though: Bchat message polling works by
            ...
            for id, bchat_id, timestamp, data, signature, deleted in rconn.execute(
                """
                ...
                # NB: the old database cleared the bchat ID when deleting a message (which is
            ...
            # Old BSGS has a bug where it inserts duplicate deletion tombstones (see above), but
            # this means that our seqno count might not be large enough for existing Bchat
            ...
            # there until expiry.
            #
            # Since we have to preserve the old random IDs for existing Bchat clients (until
            ...
            # "public_key" TEXT field containing the bchat id.

            imported_bans = 0
            for (bchat_id,) in rconn.execute("SELECT public_key FROM block_list"):
                ins_user(bchat_id)
                db.query(
                    """
            ...
            imported_mods = 0
            for (bchat_id,) in rconn.execute("SELECT public_key from moderators"):
                ins_user(bchat_id)
                db.query(
                    """
            ...
            for bchat_id, last_active in rconn.execute(
                """
