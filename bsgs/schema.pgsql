...
-- Trigger to handle required updates after a message gets deleted (in the BSGS context: that is,
...
-- This table tracks unblinded bchat ids in user_permission (and related) rows that need to be
...
-- ids are added by raw bchat ID (e.g. when adding a moderator by bchat id).
