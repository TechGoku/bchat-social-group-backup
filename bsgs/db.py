...
def database_init(create=None, upgrade=True):
    ...
    if 'messages' not in metadata.tables:
        ...
        if engine.name == "sqlite":
            conn.connection.executescript(importlib.resources.read_text('bsgs', 'schema.sqlite'))
        elif engine.name == "postgresql":
            cur = conn.connection.cursor()
            cur.execute(importlib.resources.read_text('bsgs', 'schema.pgsql'))
            cur.close()
        else:
...
def create_admin_user(dbconn):
    ...
    command-line, and give it the server's x25519 pubkey (with ff prepended, *not* bd) as a fake
    default session_id.
    """
    query(
        """
    ...
    with transaction(dbconn):
        for uid, sid in query(
            ...
            SELECT id, session_id FROM users WHERE id IN (
                ...
                UNION
                SELECT id FROM users WHERE session_id LIKE 'bd%' AND (admin OR moderator OR banned)
                EXCEPT
...
def init_engine(*args, **kwargs):
    ...
    Keyword arguments:
    bsgs_preinit - a callable to invoke after setting up `engine` but before calling
    `database_init()`.
    bsgs_skip_init - optional boolean; if specified and True then database_init() will not be
    ...
    skip_init = kwargs.pop('bsgs_skip_init', False)
    preinit = kwargs.pop('bsgs_preinit', None)
    ...
    if engine.name == "sqlite":
        ...
        if sqlite3.sqlite_version_info < (3, 25, 0):
            raise RuntimeError(
                ...
                "is too old for pybsgs (3.25.0+ required)!"
            )
