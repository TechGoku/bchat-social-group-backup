...
def server_url(room):
    # TODO: Once Bchat supports it, prefix this with /r/ so that BSGS pseudo-URLs for Bchat
...
SIGNATURE_SIZE = 64
SESSION_ID_SIZE = 33
# Size returned by make_legacy_token (assuming it is given a standard 66-hex (33 byte) bchat id):
...
def legacy_convert_time(float_time):
    """take a float unix timestamp and convert it into something legacy Bchat likes"""
...
def remove_session_message_padding(data: bytes):
    """Removes the custom padding that Bchat may have added.  Returns the unpadded data."""
    ...
    # the whole thing (without the 0's stripped off).  Bchat code has a comment "This is dumb"
...
def add_session_message_padding(data: Union[bytes, memoryview], length):
    """Adds the custom padding that Bchat delivered the message with (and over which the signature
