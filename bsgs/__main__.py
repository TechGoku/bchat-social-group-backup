...
Examples:
    ...
    # Add new room 'dev':
    python3 -mbsgs --add-room dev --name 'DEVELOPER'

    #Add new admin to a room 'dev':
    python3 -mbsgs --rooms dev --admin --addmoderators bd0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef

    # Add 2 admins to each of rooms 'dev' and 'meeting':
    python3 -mbsgs --rooms abc meeting --admin --add-moderators bd0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef bd00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff

     # Add a global moderator visible as a moderator of all rooms:
    python3 -mbsgs --add-moderators bd0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef --rooms=+ --visible

     # List room info:
    python3 -mbsgs -L

A bsgs.ini will be loaded from the current directory, if one exists.  You can override this by
specifying a path to the config file to load in the BSGS_CONFIG environment variable.

""",  # noqa: E501
    formatter_class=RawDescriptionHelpFormatter,
)

ap.add_argument('--version', '-V', action='version', version=f'PyBSGS {version}')
...
actions.add_argument(
    ...
    nargs='+',
    metavar='SESSIONID',
    help="Add the given Bchat ID(s) as a moderator of the room given by --rooms",
)
actions.add_argument(
    '--delete-moderators',
    nargs='+',
    metavar='SESSIONID',
    help="Delete the the given Bchat ID(s) as moderator and admins of the room given by --rooms",
)
ap.add_argument(
    '--admin',
    action='store_true',
...
actions.add_argument(
    ...
    "normally not required: database upgrades are performed automatically during bsgs daemon "
    "startup.",
)
actions.add_argument(
    "--check-upgrades",
...
try:
    ...
    db.init_engine(bsgs_skip_init=True)
...
BSGS initialization failed: {e}.


Perhaps you need to specify a BSGS_CONFIG path or use one of the --upgrade/--initialize options?
...
if args.initialize:
    ...
    for a in args.add_moderators:
        if not re.fullmatch(r'bd[A-Fa-f0-9]{64}', a):
            print(f"Error: '{a}' is not a valid bchat id", file=sys.stderr)
            sys.exit(1)
