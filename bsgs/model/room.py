...
# are carried over from old BSGS).
rate_limit_size = 5
...
class Room:
    ...
    def active_users_last(self, cutoff: float):
        ...
        (`active_users` is only updated every few seconds in bsgs.cleanup), and supports values
    ...
    def get_messages_for(
        ...
        *,
        ...
        Note that data and signature are returned as bytes, *not* base64 encoded.  Bchat message
        ...
        if config.PROFANITY_FILTER and not self.check_admin(user):
            ...
            if better_profanity.profanity.contains_profanity(utils.message_body(data)):
                if config.PROFANITY_SILENT:
                    ...
                    # FIXME: can we send back some error code that makes Bchat not retry?
    ...
    def get_mods(self, user=None):
        ...
        Returns a 4-tuple of lists of bchat ids:
    ...
    def get_bans(self):
        """
        Retrieves all the bchat IDs specifically banned from this room (not including global
