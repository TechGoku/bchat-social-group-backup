...
class User:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Constructs a user from a pre-retrieved row *or* a bchat id or user primary key value.
        ...
        exist then a NoSuchUser is raised if the bchat id doesn't exist.

        try_blinding - if True and blinding is required, and a given `session_id` is given that is
        *not* blinded then attempt to look up the possible blinded versions of the bchat id and
    ...
    def _refresh(
        ...
        *,
        ...
        if session_id is not None:
            if try_blinding and config.REQUIRE_BLIND_KEYS and session_id.startswith('bd'):
    ...
    def _import_blinded(self, session_id):
        ...
        if not session_id.startswith('bd'):
            return
    ...
    def __str__(self):
        ...
        if len(self.session_id) != 66:
            # Something weird (e.g. the "deleted" id from an old bsgs import), just print directly
    ...
    def verify(self, *, message: bytes, sig: bytes):
        """verify signature signed by this bchat id
    ...
    def find_blinded(self):
        """
        Attempts to look up the blinded User associated with this (unblinded) bchat id.
        ...
        if not self.session_id.startswith('bd'):  # Mainly here to catch the SystemUser
            return None
    ...
    @property
    def is_blinded(self):
        """True if the user's bchat id is a derived key"""
        return self.session_id.startswith('15')

    @property
    def system_user(self):
        """True if (and only if) this is the special BSGS system user
