...
if 'BSGS_PGSQL' in os.environ and os.environ['BSGS_PGSQL']:
    try:
        import psycopg as pg

        conn = pg.connect(os.environ['BSGS_PGSQL'], autocommit=True)
    except ModuleNotFoundError:
        import psycopg2 as pg

        conn = pg.connect(os.environ['BSGS_PGSQL'])
        conn.autocommit = True
else:
    pg = None
    import sqlite3

    conn = sqlite3.connect('file:bsgs.db?mode=ro', uri=True)
