...
for i in range(trials):
    ...
    A_xpk = s.to_curve25519_private_key().public_key.encode()  # bchat id is bd + this
    ...
    #############################
    # Signing (e.g. for X-BSGS-*) with a blinded keypair ka/kA
    ...
    # Our user A above wants to send a BSGS DM to another user B:
    ...
    # For example (in reality this would come directly from the known bchat id):
    friend_xpk = sodium.crypto_sign_ed25519_pk_to_curve25519(A)

    # From the bchat id (ignoring bd prefix) we have two possible ed25519 pubkeys; the first is
    ...
    # Optimization for Bchat:
    # - because the two blinded alternatives here differ by only the sign bit you can just always
    #   force the sign bit to be 0 when looking up a blinded -> real bchat id.  That is, when you
    ...
    #############################
    # Decrypting a BSGS DM
    # Opening the box on the recipient end.

    # I receive alongside the message from bsgs (i.e. this is the blinded bchat id minus the 'bd')
    # kA=...
    ...
    sender_session_id = 'bd' + sodium.crypto_sign_ed25519_pk_to_curve25519(sender_edpk).hex()

    assert message == msg
    assert sender_edpk == A
    assert sender_session_id == 'bd' + A_xpk.hex()
