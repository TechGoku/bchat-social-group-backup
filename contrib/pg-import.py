...
# Script to copy an sqlite database into a postgresql database.  To fully do this with bsgs:
# - stop bsgs
# - run this script
# - update the bsgs.ini to change the db to postgres
# - start bsgs
#
...
ap.add_argument('bsgs_db', type=str, nargs=1, help='SQLite database filename to import from')
...
pg_schema = importlib.resources.read_text('bsgs', 'schema.pgsql')

old = sqlite3.connect(f"file:{args.bsgs_db[0]}?mode=ro", uri=True)
