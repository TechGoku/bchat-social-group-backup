...
def build_payload(inner_json, inner_body=None, *, v, enc_type, outer_json_extra={}):
    """Encrypt and encode a payload for bsgs"""
    ...
    # [N][junk]{json}
    # where we load the fields for the last hop *and* the fields for bsgs into the json.
    outer_json = {
        "host": "localhost",
        "port": 80,
        "protocol": "http",
        "target": f"/beldex/v{v}/lsrpc",
...
def test_v3_authenticated(room, mod, client):
    ...
    # Construct an onion request for /room/test-room
    req = {'method': 'GET', 'endpoint': '/room/test-room'}
    req['headers'] = auth.x_bsgs(mod.ed_key, crypto.server_pubkey, req['method'], req['endpoint'])
    data = build_payload(req, v=3, enc_type="xchacha20")
