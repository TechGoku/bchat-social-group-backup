...
import bsgs.model.exc
from bsgs import utils
import bsgs.config
import urllib
...
def test_owned_files(client, room, room2, user, admin):
    ...
    r = bsgs_post_raw(client, f'/room/{room.token}/file', filedata, user, extra_headers=headers)
    ...
    r = bsgs_post(client, f'/room/{room.token}/message', post_info, user)
    assert r.status_code == 201
    assert 'id' in r.json
    ...
    r = bsgs_put(client, f'/room/{room.token}/message/{post_id}', new_post_info, user)
    ...
    r = bsgs_put(client, f'/room/{room.token}/message/{post_id}', post_info, user)
    assert r.status_code == 200
    f3 = File(id=f3.id)
    ...
    room_img = r.json['id']
    assert r.status_code == 201
    r = bsgs_put(client, f'/room/{room.token}', {'image': room_img}, admin)
    assert r.status_code == 200
    ...
    r = bsgs_delete(client, f'/room/{room.token}/message/{post_id}', user)
    assert r.status_code == 200
    ...
    with pytest.raises(bsgs.model.exc.NoSuchFile):
        f1 = File(id=f1.id)
    with pytest.raises(bsgs.model.exc.NoSuchFile):
        f2 = File(id=f2.id)
...
def test_no_file_crosspost(client, room, room2, user, global_admin):
    ...
    r = bsgs_post_raw(client, f'/room/{room2.token}/file', filedata, user, extra_headers=headers)
    assert r.status_code == 201
    f = File(id=r.json['id'])
    ...
    r = bsgs_put(client, f'/room/{room.token}', {'image': f.id}, global_admin)
...
def _file_upload(client, room, user, *, unsafe=False, utf=False, filename):
    ...
    assert id != 0
    r = bsgs_get(client, f'/room/{room.token}/file/{id}', user)
    assert r.status_code == 200
    ...
    f = File(id=id)
    if unsafe or utf:
        exp_path = f'{id}_' + re.sub(bsgs.config.UPLOAD_FILENAME_BAD, "_", filename)
    else:
...
def test_file_upload_banned_user(client, room, banned_user):
    url_post = f"/room/{room.token}/file"
    r = bsgs_post_raw(client, url_post, random(1024), banned_user)
...
def test_file_not_found(client, room, user, banned_user):
    ...
    r = bsgs_get(client, url_get, banned_user)
    assert r.status_code == 403
...
def test_file_read_false(client, room, user, mod):
    ...
    r = bsgs_get(client, f'/room/{room.token}/file/{id}/{filename}', user)
    assert r.status_code == 403
