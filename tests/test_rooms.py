import pytest
import time
import bsgs.model.exc as exc
from bsgs.model.room import Room, get_rooms
...
def test_pinning(room, room2, user, mod, admin, global_admin, no_rate_limit):
    bsgs = [room.add_post(user, f"data {i}".encode(), pad64(f"sig {i}")) for i in range(1, 10)]

    with pytest.raises(exc.BadPermission):
        room.pin(bsgs[3]['id'], user)
    with pytest.raises(exc.BadPermission):
        room.pin(bsgs[4]['id'], mod)
    room.pin(bsgs[5]['id'], admin)
    assert bsgs[5]['id'] == 6
