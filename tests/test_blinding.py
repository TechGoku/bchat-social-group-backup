from bsgs import crypto
from bsgs.model.room import Room
from bsgs.model.user import User
...
def test_blinded_key_derivation(seed_hex, blinded_id_exp):
    """
    Tests that we can successfully compute the blinded bchat id from the unblinded bchat id.
    ...
    session_id = 'bd' + s.to_curve25519_private_key().public_key.encode().hex()
    blinded_id = 'bd' + kA.hex()
...
def test_blinded_transition(
    ...
    with config_override(REQUIRE_BLIND_KEYS=True):
        ...
        r = client.get(
            '/capabilities',
            headers=x_bsgs(user.ed_key, crypto.server_pubkey, 'GET', '/capabilities', blinded=True),
        )
