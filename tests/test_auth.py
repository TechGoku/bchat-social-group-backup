...
from bsgs.crypto import server_pubkey
from bsgs.routes.auth import user_required
from auth import x_bsgs_raw, x_bsgs
import bsgs.utils
...
def test_auth_basic(client, db):
    ...
    B = server_pubkey
    session_id = 'bd' + a.verify_key.to_curve25519_public_key().encode().hex()

    # Basic auth:
    print(x_bsgs(a, B, 'GET', '/auth_test/whoami'))
    r = client.get("/auth_test/whoami", headers=x_bsgs(a, B, 'GET', '/auth_test/whoami'))
    ...
    hh = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    assert hh['X-BSGS-Nonce'].endswith('=')
    hh['X-BSGS-Nonce'] = hh['X-BSGS-Nonce'].rstrip('=')
    assert not hh['X-BSGS-Nonce'].endswith('=')
    ...
    r = client.get(
        "/auth_test/whoami", headers=x_bsgs(a, B, 'GET', '/auth_test/whoami', b64_nonce=False)
    )
    assert r.status_code == 200
    assert r.json == {"user": {"uid": 1, "session_id": session_id}}
    ...
    r = client.get(
        "/auth_test/whoami", headers=x_bsgs(a, B, 'GET', '/auth_test/whoami', timestamp_off=86399)
    )
    assert r.status_code == 200
    assert r.json == {"user": {"uid": 1, "session_id": session_id}}
...
def test_auth_required(client, db):
    ...
    r = client.get(
        "/auth_test/auth_required", headers=x_bsgs(a, B, 'GET', '/auth_test/auth_required')
    )
    assert r.status_code == 200
    assert r.json == {"user": {"uid": 1, "session_id": session_id}}
    ...
    assert r.data == b'X-BSGS-* request authentication required'
    ...
    r = client.post(
        ...
        headers=x_bsgs(a, B, 'POST', '/auth_test/auth_required', body),
    )
    assert r.status_code == 200
    assert r.json == {
...
def test_auth_banned(client, global_admin, user, db):
    ...
    r = client.post(
        '/auth_test/auth_required',
        data=b'[1,2,3]',
        headers=x_bsgs(a, B, 'POST', '/auth_test/auth_required', b'[1,2,3]'),
    )
    assert r.status_code == 403
    assert r.data == b'Banned'
...
def test_auth_malformed(client, db):
    ...
    headers, n, ts, sig = x_bsgs_raw(a, B, 'GET', '/auth_test/whoami')
    sig = sig[0:10] + bytes((sig[10] ^ 0b100,)) + sig[11:]
    headers['X-BSGS-Signature'] = bsgs.utils.encode_base64(sig)

    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 401
    assert r.data == b'Invalid authentication: X-BSGS-Signature verification failed'

    # Wrong hash size:
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Signature'] = headers['X-BSGS-Signature'][:-4]
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert r.data == b'Invalid authentication: X-BSGS-Signature is not base64[88]'

    # Missing a header
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    del headers['X-BSGS-Timestamp']
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert r.data == b'Invalid authentication headers: missing 1/4 required X-BSGS-* headers'

    # Empty header
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Timestamp'] = ''
    headers['X-BSGS-Nonce'] = ''
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert r.data == b'Invalid authentication headers: missing 2/4 required X-BSGS-* headers'

    # Wrong path
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoareu')
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 401
    assert r.data == b'Invalid authentication: X-BSGS-Signature verification failed'
    ...
    # Malformed headers
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Timestamp'] = headers['X-BSGS-Timestamp'] + 'a'
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert r.data == b'Invalid authentication: X-BSGS-Timestamp is not a valid timestamp'

    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = headers['X-BSGS-Nonce'] + '='  # Invalid base64 (too much padding)
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
        == b'Invalid authentication: X-BSGS-Nonce must be 16 bytes (encoded as base64 or hex)'
    )

    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = headers['X-BSGS-Nonce'][:-1]  # Invalid base64 (wrong padding)
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
    ...
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = headers['X-BSGS-Nonce'][:-6]  # chop off 2 padding + last 4 chars
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
    ...
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = bsgs.utils.decode_base64(headers['X-BSGS-Nonce']).hex()[:-1]
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
    ...
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = bsgs.utils.decode_base64(headers['X-BSGS-Nonce']).hex()[:-2]
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
    ...
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami')
    headers['X-BSGS-Nonce'] = bsgs.utils.decode_base64(headers['X-BSGS-Nonce']).hex() + 'ff'
    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert (
        r.data
    ...
    assert r.data == b'Invalid authentication: X-BSGS-Nonce cannot be reused'

    # Bad timestamps
    r = client.get(
        "/auth_test/whoami", headers=x_bsgs(a, B, 'GET', '/auth_test/whoami', timestamp_off=86402)
    )
    assert r.status_code == 425
    assert r.data == b'Invalid authentication: X-BSGS-Timestamp is too far from current time'
...
def test_auth_batch(client, db):
    ...
    reqs = [
        ...
        {"method": "POST", "path": "/auth_test/auth_required", "b64": bsgs.utils.encode_base64(hi)},
        {
            "method": "POST",
    ...
    inner_h1 = x_bsgs(a2, B, 'GET', '/auth_test/whoami')
    inner_h1['X-Foo'] = 'bar'
    inner_h2 = x_bsgs(a2, B, 'POST', '/auth_test/auth_required', hi)
    reqs = [
        ...
        {"method": "GET", "path": "/auth_test/whoami"},
        {
            ...
            "path": "/auth_test/auth_required",
            "headers": inner_h2,
            "b64": bsgs.utils.encode_base64(hi),
        },
    ]
    body = json.dumps(reqs).encode()
    headers = x_bsgs(a, B, 'POST', '/batch', body)
...
def test_auth_legacy(client, db, admin, user, room):
    ...
    # gets ignored when we use X-BSGS-*.
    raw_token = bsgs.utils.make_legacy_token(admin.session_id)
    token = bsgs.utils.encode_base64(raw_token)
    ...
    bad_token = bsgs.utils.encode_base64(bytes((raw_token[0] ^ 1,)) + raw_token[1:])
    r = client.post(
        "/legacy/block_list",
    ...
    S2 = 'bd' + a2.verify_key.to_curve25519_public_key().encode().hex()
    r = client.post(
        "/legacy/block_list",
        headers={"Room": room.token, "Authorization": token},
        json={"public_key": S2},
    ...
    utoken = bsgs.utils.encode_base64(bsgs.utils.make_legacy_token(user.session_id))
    ...
    # Same, but now use X-BSGS-*
    h = x_bsgs(user.ed_key, B, 'GET', '/legacy/block_list')
    h['Room'] = room.token
    r = client.get("/legacy/block_list", headers=h)
    assert r.status_code == 200
    assert r.json == {"status_code": 200, "banned_members": [user.session_id]}

    # Now use X-BSGS-* for the second user, but pass an Authentication header for the first user
    # (which should be ignored with X-BSGS-* headers present).
    h = x_bsgs(a2, B, 'GET', '/legacy/block_list')
    h['Room'] = room.token
    r = client.get("/legacy/block_list", headers=h)
    assert r.status_code == 200
    assert r.json == {"status_code": 200, "banned_members": [S2]}

    # Remove the bans as admin, with X-BSGS
    rh = {"Room": room.token}
    body = json.dumps(
        [
...
def test_small_subgroups(client, db):
    ...
    assert headers['X-BSGS-Pubkey'].startswith('bd')
    A = bytes.fromhex(headers['X-BSGS-Pubkey'][2:])
    ...
    headers['X-BSGS-Pubkey'] = 'bd' + Abad.hex()

    r = client.get("/auth_test/whoami", headers=headers)
    assert r.status_code == 400
    assert r.data == b'Invalid authentication: given X-BSGS-Pubkey is not a valid Ed25519 pubkey'

    # Now try with a blinded id:
    headers = x_bsgs(a, B, 'GET', '/auth_test/whoami', blinded=True)
    assert headers['X-BSGS-Pubkey'].startswith('bd')
    A = bytes.fromhex(headers['X-BSGS-Pubkey'][2:])
