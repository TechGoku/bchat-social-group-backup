...
from bsgs import web  # noqa: E402
from bsgs.model.room import Room  # noqa: E402
from bsgs.model.user import SystemUser  # noqa: E402

import bsgs.omq  # noqa: E402
    ...
    if pgsql:
        if sqlalchemy.__version__.startswith("1.3."):
            ...
            with conn.cursor() as cur:
                cur.execute("CREATE SCHEMA IF NOT EXISTS bsgs_tests")
            conn.commit()

            pgsql_w_schema = pgsql + ('&' if '?' in pgsql else '?')
            pgsql_w_schema += 'options=--search_path%3Dbsgs_tests%2Cpublic'
            ...
            if not request.config.getoption("--pgsql-no-drop-schema"):
                ...
                with conn.cursor() as cur:
                    cur.execute("DROP SCHEMA bsgs_tests CASCADE")
                conn.commit()
...
def db(request, pgsql):
    ...
    """
    d = tempfile.mkdtemp(prefix='tmp_pybsgs')
    _tempdirs.add(d)
    config.UPLOAD_PATH = d
    trace = request.config.getoption("--sql-tracing")

    from bsgs import db as db_
    ...
    if pgsql:
        ...
        def pg_setup_schema():
            ...
            def setup_schema(dbapi_connection, connection_record):
                ...
                nonlocal first
                if first:
                    cursor.execute("DROP SCHEMA IF EXISTS bsgs_tests CASCADE")
                    first = False
                cursor.execute("CREATE SCHEMA IF NOT EXISTS bsgs_tests")
                cursor.execute("SET search_path TO bsgs_tests, public")
                cursor.close()

                dbapi_connection.autocommit = existing_autocommit

        db_.init_engine(pgsql, echo=trace, bsgs_preinit=pg_setup_schema)

    else:
        sqlite_uri = f'file:bsgs_testdb{db_counter_}?mode=memory&cache=shared'
    ...
    if (
        ...
        db_.query("DROP SCHEMA bsgs_tests CASCADE")
...
def no_rate_limit():
    ...
    import bsgs.model.room as mroom
